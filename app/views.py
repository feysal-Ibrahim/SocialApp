...
from app.forms import AuthenticateForm , UserCreateForm , RibbitForm
from app.models import Ribbit
...
def users(request , username="" , ribbit_form=None):
    ...
    ribbit_form=ribbit_form or RibbitForm( )
    return render( request , 'profile.html' , {'obj': obj , 'next_url': '/users/' , 'ribbit_form': ribbit_form ,
    ...
    if request.method == "POST":
        ...
        if unfollow_id:
            try:
                ...
                request.user.profile.unfollow.add( user.profile )
            except ObjectDoesNotExist:
                return redirect( '/users/' )
    return redirect( '/users/' )